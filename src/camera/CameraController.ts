 ...
 *
 * The camera is presentation-only. Authoritative gameplay state remains in WorldState.
 * Tracking uses the exact critically damped solution shared with simulation consumers so
 * behaviour is stable across variable render schedules and does not depend on 60 Hz tuning.
 */
...
export class CameraController {
  ...
  update(
    ...
    const safeDt = THREE.MathUtils.clamp(dt, 0, MAX_RENDER_DT);
    const preset = MODE_PRESETS[settings.mode];
