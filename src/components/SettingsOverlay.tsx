    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setSettingsOpen(false)} className="rounded-lg border border-white/10 bg-white/5 p-2 text-white/70 hover:bg-white/10" aria-label="Close settings">
            <X size={16} />
          </button>
        </div>
        ...
        <section className="mb-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {modes.map((mode) => (
              <button key={mode.id} type="button" onClick={() => setCameraMode(mode.id)} className={`rounded-lg border px-3 py-2 text-left transition-colors ${cameraMode === mode.id ? 'border-blue-400/50 bg-blue-500/15 text-blue-200' : 'border-white/10 bg-white/5 text-white/70 hover:bg-white/10'}`}>
        ...
        {commentaryEnabled && (
          <section className="mb-5 space-y-3">
            <div>
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-white/40">Commentary voice</p>
              <select value={commentaryVoice} onChange={(e) => setCommentaryVoice(e.target.value as typeof commentaryVoice)} className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white/80">
                {COMMENTARY_VOICES.map((voice) => (
                  <option key={voice.id} value={voice.id} className="bg-slate-900">{voice.label}</option>
                ))}
              </select>
            </div>
            <label className="block rounded-lg border border-white/10 bg-white/5 px-3 py-2.5">
              <span className="mb-2 block text-sm text-white/80">Commentary volume</span>
              <input type="range" min={0} max={1} step={0.05} value={commentaryVolume} onChange={(e) => setCommentaryVolume(Number(e.target.value))} className="w-full accent-blue-500" />
            </label>
          </section>
        )}

        <section className="mb-5">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-white/40">Dev — match length</p>
          <div className="flex gap-2">
            {([90, 180] as const).map((seconds) => (
              <button key={seconds} type="button" onClick={() => setDevMatchDuration(seconds)} className={`flex-1 rounded-lg border py-2 text-sm font-medium ${devMatchDuration === seconds ? 'border-blue-400/50 bg-blue-500/15 text-blue-200' : 'border-white/10 bg-white/5 text-white/60'}`}>
        ...
        <section>
          ...
          <div className="flex gap-2">
            {zoomLevels.map((level) => (
              <button key={level.id} type="button" onClick={() => setZoomIntensity(level.id)} className={`flex-1 rounded-lg border py-2 text-sm font-medium ${zoomIntensity === level.id ? 'border-blue-400/50 bg-blue-500/15 text-blue-200' : 'border-white/10 bg-white/5 text-white/60'}`}>
                {level.label}
              </button>
            ))}
          </div>
